...
```
Browser (sadd-website.html — Wave 1: devices + advfirewall screens; Wave 2: about's version row + advlogs' Recent Activity list; Wave 3: settings' Wi-Fi/guest rows + guest's on/off toggle; Wave 4: adblock's stat/switch + advnetwork's VLAN list)
...
- Every other screen (all 40 remaining desktop screens, all 16 mobile screens) is untouched — still static demo data, exactly as it is today.
...
`/cgi-bin/api/*` has no authentication of any kind, and `docker-compose.yml`'s `ports: "8081:80"` binds to all interfaces (`0.0.0.0`), not just `localhost` — so anyone who can reach the host machine on the local network can add or delete real firewall rules, flip the real guest Wi-Fi network on/off, or flip real DNS ad blocking on/off (three independent write endpoints as of Wave 4: Wave 1's `POST/DELETE /cgi-bin/api/firewall-rules`, Wave 3's `POST /cgi-bin/api/wifi`, and Wave 4's `POST /cgi-bin/api/adblock`), with a plain `curl`, no login required. This is acceptable **only** because this pilot is an explicitly local, single-user dev/test tool with no intended multi-user or untrusted-network exposure (see Non-goals below) — it is not something to carry forward into any later wave or real deployment without addressing.

The same lack of authentication also applies to every **read-only** endpoint (`/api/devices`, `/api/logs`, `/api/system-info`, and, as of Wave 4, `/api/vlans`) — anyone reachable on the local network can silently read real connected-device presence/MAC/IP data, real syslog/dropbear log lines, real distro/hardware/uptime info, and real VLAN/network topology, with a plain `curl`, no login required. This is disclosure rather than mutation, but it's still real information about a real (if virtual) router, and falls under the same acceptable-only-because-local-single-user-dev-tool reasoning above — not something to carry forward as-is either.
...
**Wave 5 (scoped 2026-09-01 against a live feasibility survey — `docker/facts.md` §13 — which materially narrowed this from the original title-only listing below):**
