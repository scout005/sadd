...
Drive as much of this prototype as genuinely can be by a real, running OpenWrt instance, instead of hardcoded demo data. The full target is documented below as a roadmap of "waves," each one a self-contained slice that gets its own implementation plan when its turn comes. **Wave 1** (done) covers two screens on the desktop prototype (`sadd-website.html`): **Devices** and **Firewall & Port Forwarding**. **Wave 2** (done) covers **About**'s version row and **Diagnostics & Logs**' "Recent activity" list. **Wave 3** (done) covers **Settings**' "Wi-Fi name & password"/"Guest network" row values and **Guest Wi-Fi**'s on/off toggle. **Wave 4** (done) covers **Ad Blocking**'s toggle + real blocked-count and **Network & VLANs**' VLAN list. Everything below Wave 4 in the roadmap is sequenced but not yet detailed at the same level; each wave gets its own design pass (reusing this document's architecture) immediately before it's built, since exact implementation details depend on what's actually true inside the running container by that point.
...
**Wave 5 (harder simulation, needs real generated traffic/hardware-adjacent behavior):**
...
Wave 5's items are ordered by rough complexity, not obligation — the actual order when we get there can be re-prioritized based on what's most useful to test at the time.
