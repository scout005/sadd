...
**Post-implementation correction (whole-feature integration code review)**: the design above (step 3, "add the just-read counter's bytes") turned out to have a real gap this spec didn't anticipate — it implicitly assumed the sweep's own step-4 `fw4 reload` always succeeds, and never accounted for that call *failing*. A failed reload leaves the raw counter un-reset, so the next tick's "just-read counter" would include traffic *already* added the tick before — a genuine overcount, not the undercount this spec promised. Fixed by tracking each device's last-observed raw counter value (a new persisted file, not date-keyed) and accumulating the delta against it rather than the raw value itself — correct regardless of whether any given `fw4 reload` succeeds. See `docker/provision/lib/qos-bandwidth-sweep.sh`'s own header comment and `docker/README.md`'s file note for the full fix, and confirmed live for both directions (a simulated failed reload no longer double-counts; the disclosed limitation above — an unrelated endpoint's *successful* reload between ticks — still behaves exactly as originally designed).
