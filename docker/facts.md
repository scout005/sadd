...
## 19. Wave 9 (Bandwidth Used Today redesign) pre-investigation — `nft reset` is a no-op on this VM; `fw4 reload` fully regenerates the table instead

Confirmed live (2026-09-02, VM already provisioned through Wave 8), while brainstorming a redesign of the previously-deferred "Bandwidth used today" feature (see §18's honest "still deferred" conclusion and `docs/superpowers/specs/2026-09-02-qos-bandwidth-today-design.md` for the resulting scope change).

**`nft reset` does not work on this VM — confirmed by direct testing, not assumed from documentation**: nftables v1.0.8, kernel 5.15.167. Created a throwaway rule with a known non-zero counter (`nft add rule inet resettest c counter packets 42 bytes 9999`), then tried every documented reset form against it:
- `nft reset rules table inet resettest` — command accepted, no error, but the counter was **still 42/9999** afterward, unchanged.
- `nft reset rule inet resettest c handle <N>` (resetting by explicit rule handle) — same result, no error, counter unchanged.
- A named counter object (`nft add counter inet resettest mycounter`, referenced via `counter name mycounter` in the rule) plus `nft reset counters table inet resettest` — same result.

Re-confirmed against genuinely live, growing traffic (not just a static preset value): created a counter-only rule on an `output`-hook chain (`type filter hook output priority 0;`, real router-originated traffic, easy to generate via `ping -c N -W 1 8.8.8.8` — the target's unreachability doesn't matter, the packet still traverses the hook), watched the counter grow across real pings (1 → 11 → 23 packets), then ran `nft reset rule ... handle <N>` — the counter kept growing across the reset attempt exactly as if nothing had happened, no reset effect at all. Root cause not investigated further (likely a kernel/nft-userspace version gap specific to this environment) — not fixable from this project's side, and not necessary to fix given the working alternative below.

**`fw4 reload` DOES reliably zero every counter in the table — but as a side effect of a full table rebuild, not a targeted reset**: proven by injecting a throwaway rule directly into the *live, real* `inet fw4` table via raw `nft` (bypassing `uci` entirely — `nft add rule inet fw4 mangle_forward counter packets 500 bytes 60000 comment "scratch-test-rule"`), confirming it appeared in `nft list chain inet fw4 mangle_forward`, then running `fw4 reload` (the same command every write endpoint in this project already calls) and confirming the rule was **completely gone** afterward — not reset, deleted, because `fw4 reload` regenerates the entire table from `uci` config from scratch every time, and my manually-injected rule was never in `uci` to begin with. This proves any legitimately `uci`-defined rule's own inline counter also restarts at 0 on every `fw4 reload`, since the whole rule (counter included) is recreated fresh from config, not incrementally patched.

**Scoping implication, the load-bearing one**: `fw4 reload` is not a narrow "reset the bandwidth counter" primitive — it's called by *every* existing write endpoint in this app (`/api/device-pause`, `/api/qos-priority` itself, `/api/device-bedtime`, `/api/firewall-rules`, etc.), so any of them firing zeroes every counter in the table as a side effect, not just a purpose-built daily-reset cron. A naive "reset once at UTC midnight" design would silently also reset (from the user's perspective, lose accumulated data) any time an unrelated feature elsewhere in the app happens to run that same day. The chosen design (`docs/superpowers/specs/2026-09-02-qos-bandwidth-today-design.md`) works around this by polling every 5 minutes and accumulating into a persisted per-device-per-day file rather than trusting the raw counter to survive a full day untouched — with an honestly disclosed, real (not hypothetical) limitation: a reload from an unrelated endpoint between two sweep ticks still causes an undercount for that window, since the sweep can't read what's already been zeroed by someone else's action.

**A useful side effect of this finding for the storage design**: since a new UTC calendar date's persisted file simply doesn't exist yet, "today starts fresh" requires no hour-comparison arithmetic anywhere — a deliberate design choice to avoid Wave 7's own `bedtime-sweep.sh` busybox-ash `$(( ))` gotcha (§16) by construction, not by coincidence.
